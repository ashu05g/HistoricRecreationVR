...
public class TTS_SF_Simba : MonoBehaviour
{
    ...
    [SerializeField] private XRGrabInteractable interactableToReactivate;

    // Start is called before the first frame update
    void Start()
    {
        //avtAnimator = GetComponent<Animator>();
        //avtAnimator = GetComponentInChildren<Animator>();

        sfVoice = selectVoice.ToString().Substring(3);
        Debug.Log("You have selected voice " + sfVoice);

        //For testing
        //Say("Hello Ashutosh! How are you?");
    }
    ...
    IEnumerator PlayTTS(string mesg)
    {
        ...
        Debug.Log("TTS got the msg: " +ttsData.input);
        ...
        if (request.result == UnityWebRequest.Result.Success)
        {
            ...
            Debug.Log("TTS response size: " + request.downloadHandler.data.Length);
            Debug.Log("TTS content-type: " + request.GetResponseHeader("Content-Type"));
            System.IO.File.WriteAllBytes("Assets/last_tts_response.mp3", request.downloadHandler.data);
    ...
    IEnumerator WaitForTalkingFinished()
    {
        ...
        Debug.Log("TTS finished — re-enabling interaction");

        if (interactableToReactivate != null)
            interactableToReactivate.enabled = true;
